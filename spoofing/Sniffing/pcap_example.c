...
int main(int argc, char *argv[])
{
	...
	*(dev_name + strlen(dev_name) - 1) = '\0'; //the pcap_open_live don't take the last \n in the end
	...
	if(argc > 1){
		//concatenate all argv into one filter_exp string
		char *filter_exp = (char *) malloc(100 * sizeof(char));
		*filter_exp = '\0';
		int size = 0;
		for(int i = 1; i<argc; i++){
			size += strlen(argv[i]);
			filter_exp = (char *) realloc(filter_exp, (size + 2) * sizeof(char));
			if(i > 1) strcat(filter_exp, space);
		...
		//Piece of code taken http://www.tcpdump.org/pcap.html
